...
% Set type of tracking controller
    TTC_type = 'TypeII';
% Inititalize the tracking controller 
    delta = -1;
    epsilon = [delta; 0];                                       
...
% Update vehicle state
  p(:,end+1) = Tracker.State(1:2,end);                                      % tracker position
  psi(end+1) = Tracker.State(3,end);                                        % tracker heading
% Update desired S-T curve
    ...
    r_gamma = [rx*cos(gamma(end)+phi);                                      % spatial path
...
% Call ST tracking controller
    ...
    [Tracker.Input(:,end+1), gamma_dot(end+1), e_pos_hat, e_gamma] ... 
     = ST_Tracking_2D(t,Ts,p(:,end),psi(end),pd_hat,pd_dot_hat,pd_gamma, TTC_type, gamma_dot(end),omega);
...
% Compute position error respect to q - true target trajectory
    R_IB = [cos(psi(end)) -sin(psi(end));                                             % rotation matrix from I to B
            sin(psi(end))  cos(psi(end))];
    e_pos_q = R_IB'*(p(:,end) - pd(:,end)) - epsilon; 
