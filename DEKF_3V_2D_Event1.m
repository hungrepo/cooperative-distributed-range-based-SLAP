...
%     Omega3_fus=(Omega3+Omega2_bar)/2;
   % Check communication
             ...
             e(1)=trace(Omega1_bar*inv(Omega1))+(x1_bar-x1hat)'*Omega1_bar*(x1_bar-x1hat)+log(det(Omega1)/det(Omega1_bar))-n;
             e(2)=trace(Omega2_bar*inv(Omega2))+(x2_bar-x2hat)'*Omega2_bar*(x2_bar-x2hat)+log(det(Omega2)/det(Omega2_bar))-n;
             e(3)=trace(Omega3_bar*inv(Omega3))+(x3_bar-x3hat)'*Omega3_bar*(x3_bar-x3hat)+log(det(Omega3)/det(Omega3_bar))-n;
             d_lk=e;
  ...
  % Broadcast signal
            if  delta(1)>= 0
                Com_Sig(1)=1; q1_bar=q1; Omega1_bar=Omega1;  x1_bar=inv(Omega1_bar)*q1_bar;
                q2_fuse=(q2+q1_bar)/2;
                Omega2_fuse=(Omega2+Omega1_bar)/2;
                delta_q2_fuse=(delta_q2+delta_q1)/2;
                delta_Omega2_fuse=(delta_Omega2+delta_Omega1)/2;
            else
                Com_Sig(1)=0;
                q2_fuse=q2;
                Omega2_fuse=Omega2;
                delta_q2_fuse=delta_q2;
                delta_Omega2_fuse=delta_Omega2;
            end
            if  delta(2)>= 0
                Com_Sig(2)=1; q2_bar=q2; Omega2_bar=Omega2;  x2_bar=inv(Omega2_bar)*q2_bar;
                  q3_fuse=(q3+q2_bar)/2;
                  Omega3_fuse=(Omega3+Omega2_bar)/2;
                  delta_q3_fuse=(delta_q3+delta_q2)/2;
                  delta_Omega3_fuse=(delta_Omega3+delta_Omega2)/2;
            else
                Com_Sig(2)=0;
                q3_fuse=q3;
                Omega3_fuse=Omega3;
                delta_q3_fuse=delta_q3;
                delta_Omega3_fuse=delta_Omega3;
            end
            if  delta(3)>= 0
                Com_Sig(3)=1; q3_bar=q3; Omega3_bar=Omega3;  x3_bar=inv(Omega3_bar)*q3_bar;
                    q1_fuse=(q1+q3_bar)/2;
                    Omega1_fuse=(Omega1+Omega3_bar)/2;
                    delta_q1_fuse=(delta_q1+delta_q3)/2;
                    delta_Omega1_fuse=(delta_Omega1+delta_Omega3)/2;
            else
                Com_Sig(3)=0;
                q1_fuse=q1;
                Omega1_fuse=Omega1;
                delta_q1_fuse=delta_q1;
                delta_Omega1_fuse=delta_Omega1;
            end
end
...
% correct    
    q1_cor=q1_fuse+delta_q1_fuse;
    q2_cor=q2_fuse+delta_q2_fuse;
    q3_cor=q3_fuse+delta_q3_fuse;
    
    Omega1_cor=Omega1_fuse+delta_Omega1_fuse;
    Omega2_cor=Omega2_fuse+delta_Omega2_fuse;
    Omega3_cor=Omega3_fuse+delta_Omega3_fuse;  
